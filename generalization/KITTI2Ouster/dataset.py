...
class Ouster64Dataset(Data.Dataset):
    DATA_FILES = {
    ...
    def __init__(self,
        ...
        self.prepare_ouster64_ply(split=self.split)

    def prepare_ouster64_ply(self, split='train'):
    ...
    def __getitem__(self, index):
        ...
        fname0 = self._get_ouster_fn(drive, t0)
        fname1 = self._get_ouster_fn(drive, t1)

        # XYZ and reflectance
        o3d_cloud0 = o3d.io.read_point_cloud(fname0)
        o3d_cloud1 = o3d.io.read_point_cloud(fname1)
        xyzr0 = np.asarray(o3d_cloud0.points, dtype=np.float32)
        xyzr1 = np.asarray(o3d_cloud1.points, dtype=np.float32)
        ...
        if key not in Ouster64_cache:
            if not os.path.exists(filename):
                ...
                M2 = np.load(filename)
            Ouster64_cache[key] = M2
        else:
            M2 = Ouster64_cache[key]
        trans = M2
    ...
    def get_video_odometry(self, drive, indices=None, ext='.txt', return_all=False):
        data_path = self.pc_path + '/%s/poses.txt' % drive # To use each datasets' GT pose
        if data_path not in Ouster64_cache:
            Ouster64_cache[data_path] = np.genfromtxt(data_path)
        if return_all:
            return Ouster64_cache[data_path]
        else:
            return Ouster64_cache[data_path][indices]
    ...
    def _get_ouster_fn(self, drive, t):
